...
import com.hwc.abllib.callback.AnisCallBack;
import com.hwc.abllib.callback.GestureCallBack;
...
public class AblViewUtil {
    ...
    public static AccessibilityNodeInfo findById(
        ...
        if (parent != null) {
            List<AccessibilityNodeInfo> accessibilityNodeInfos = parent.findAccessibilityNodeInfosByViewId(id);
    ...
    public static List<AccessibilityNodeInfo> findById(
            AccessibilityNodeInfo parent,
            String id, @Nullable AnisCallBack anisCallBack) {
        if (parent != null) {
            List<AccessibilityNodeInfo> accessibilityNodeInfos = parent.findAccessibilityNodeInfosByViewId(id);
            if (accessibilityNodeInfos != null &&
                    !accessibilityNodeInfos.isEmpty()) {
    ...
    public static AccessibilityNodeInfo findByText(AccessibilityNodeInfo parent, String text, int position) {
        if (parent != null) {
            List<AccessibilityNodeInfo> accessibilityNodeInfos = parent.findAccessibilityNodeInfosByText(text);
            if (accessibilityNodeInfos != null
                    && !accessibilityNodeInfos.isEmpty()
                    && position < accessibilityNodeInfos.size()) {
                return accessibilityNodeInfos.get(position);
    ...
    public static void scrollVertical(
        ...
        dispatch_gesture(new float[]{screenWidth >> 1, start},
     ...
     */
    public static void scrollVertical(
            float startY, float stopY,
            long startTime, long duration,
            @Nullable GestureCallBack callback) {
        int screenWidth = ScreenUtils.getScreenWidth();
        dispatch_gesture(new float[]{screenWidth >> 1, startY},
    ...
    public void scrollHorizontal(
        ...
        dispatch_gesture(new float[]{start, screenHeight >> 1},
     ...
     * @param callback  回调
     */
    public void scrollHorizontal(int startX, int stopX, long startTime,
                                 long duration, @Nullable GestureCallBack callback) {
        int screenHeight = ScreenUtils.getScreenHeight();
        dispatch_gesture(new float[]{startX, screenHeight >> 1},
                new float[]{stopX, screenHeight >> 1}, startTime,
                duration, callback);
    }
     ...
     * @param yRatio
     */
    public static void clickScreen(int ratio,
                                   int xRatio, int yRatio,
                                   @Nullable GestureCallBack callback) {
        int screenHeight = ScreenUtils.getScreenHeight();
        int screenWidth = ScreenUtils.getScreenWidth();
        int y = (screenHeight / ratio) * yRatio;
        int x = (screenWidth / ratio) * xRatio;
        dispatch_gesture(new float[]{x, y},
                new float[]{x, y}, 100,
                50, callback);
    }
     ...
     * @param duration  持续时间
     */
    public static void clickScreen(float x, float y, long startTime, long duration,
                                   @Nullable GestureCallBack callback) {
        dispatch_gesture(new float[]{x, y},
                new float[]{x, y}, startTime,
                duration, callback);
    }
    ...
    public static boolean clickNodeInfo(AccessibilityNodeInfo nodeInfo) {
        if (nodeInfo != null) {
     ...
     * @return
     */
    public static boolean clickOpenCheckNodeInfo(AccessibilityNodeInfo nodeInfo, boolean isHandleParent) {
        if (nodeInfo != null) {
            if (!nodeInfo.isChecked()) {
                AccessibilityNodeInfo parent = nodeInfo.getParent();
                if (isHandleParent && parent != null) {
     ...
     * @return
     */
    public static boolean clickCloseCheckNodeInfo(AccessibilityNodeInfo nodeInfo, boolean isHandleParent) {
        if (nodeInfo != null) {
            if (nodeInfo.isChecked()) {
                AccessibilityNodeInfo parent = nodeInfo.getParent();
                if (isHandleParent && parent != null) {
